...
export default function BlogCard({
  ...
  return (
    <>
      <Card className={classes.root} onClick={handlePostEditOpen}>
        <CardHeader
          avatar={
          ...
          // }
          title={title}
          subheader={new Date(post_scheduled).toDateString()}
          // subheader='September 14, 2016'
        />
        <CardMedia
          className={classes.media}
          image={image?.replace("backend", "localhost/api")}
          title={title}
        />
        <CardContent>
          {parse(caption)}
